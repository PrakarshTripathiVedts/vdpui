...
import { getNotReqAbbreviationIdsImpl, updateImplNotReqAbbreviationIds } from "services/implementation.service";
...
useEffect(() => {
  ...
  const getAbbreviationsList = async () => {
    try {
        ...
        let revistionRecord = await getNotReqAbbreviationIdsImpl(revisionData.docVersionReleaseId);
...
const handleFilterByLetter = (letter) => {
  ...
  const handleDelete = async(abbreviationId) => {
    setAbbreviationId(abbreviationId);
      ...
      .sort((a, b) => a - b);
      let res = await updateImplNotReqAbbreviationIds(combinedData + '', revisionData.docVersionReleaseId + '');
      if (res && res > 0) {
        getAbbreviationsList();
        Swal.fire({
            icon: "success",
  ...
  const handleAddBack = async() => {
   ...
   const confirm = await AlertConfirmation({
    ...
    if (confirm) {
        ...
        let res = await updateImplNotReqAbbreviationIds(combinedIds + '', revisionData.docVersionReleaseId + '');
        if (res && res > 0) {
          getAbbreviationsList();
          Swal.fire({
            icon: "success",
  ...
  switch (status) {
    case 'userManualDocContent':
        return <ImplAddContentEditorComponent versionElements={revisionData} ></ImplAddContentEditorComponent>;
    default:
  return (
    <div>
        <Navbar></Navbar> 
