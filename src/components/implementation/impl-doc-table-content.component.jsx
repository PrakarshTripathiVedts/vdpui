...
import ImplDocContentDialogComponent from "./impl-doc-content-dialog.component";
import ImplAddContentEditorComponent from "./impl-add-content-editor.component";
import { getImplTableContentById, getImplTableContentList } from "services/implementation.service";

//import srsDocContentDialog from "components/SRSDocx/srs-add-content-section-dialog"



const ImplDocTableComponent = (props)=>{
    ...
    const [ProjectSelDto, setProjectSelDto] = useState(null);
       ...
       const fetchData = async () => {
        try {
            ...
            let tableContentList =  await getImplTableContentList(projectSelDtoData);
    ...
    const handleEditContent = async (contentId) => {
        let data = await getImplTableContentById(contentId);
...
switch (status) {
    case 'umDocContent':
        return <ImplAddContentEditorComponent versionElements={versionElements}></ImplAddContentEditorComponent>;
    default:
    return (
    <Box id="qms-wrapper">
        <Helmet>
  ...
  {`Doc Content for ${versionElements.projectMasterDto?.projectName ?? ''}`}
                ...
                <ImplDocContentDialogComponent open={open} onClose={() => setOpen(false)} qaqtDocData={ProjectSelDto} type={contentType} 
...
export default withRouter(ImplDocTableComponent);
