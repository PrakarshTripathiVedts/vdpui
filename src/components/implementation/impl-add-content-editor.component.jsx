...
import { Box, Breadcrumbs, Button, Grid, IconButton, Link, TextField, Tooltip, Typography, Card, CardContent, Snackbar, Alert, Dialog, DialogTitle, DialogContent, DialogActions, FormControlLabel, Switch, List, ListItem, ListItemText, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Paper } from '@mui/material';
import Navbar from "components/navbar/Navbar";
import { Helmet } from 'react-helmet';
import withRouter from "../../common/with-router";

import './impl-manual-add-doc-content.css';

import $ from 'jquery';

import AlertConfirmation from "common/AlertConfirmation.component";
import { MdAccountTree } from "react-icons/md";
import ImplDocsAddDocContentAddSectionDialog from './impl-add-content-section-dialog.component';


import ImplDocTableComponent from './impl-doc-table-content.component'

import 'summernote/dist/summernote-lite.css';
import 'summernote/dist/summernote-lite.js';
import ImplDocPrint from './implementation-doc-print.component';


import AddAbbreviationComponent from './impl-add-abbreviation-content.component';

import ImplementationDocRecordsComponent from './implementation-doc-records.component';
import { addImplSubChapterNameByChapterId, deleteImplChapterByChapterId, getImplMainChapters, getImplSubChaptersById, updateImplChapterBySnNo, updateImplChapterContent, updateImplChapterNameById, updateImplchapterPagebreakAndLandscape } from 'services/implementation.service';



const ImplAddContentEditor = (props) => {
    ...
    const getDocPDF = (versionElements) => {
        ...
        return <ImplDocPrint action='' revisionElements={versionElements} buttonType={'Button'} />;
       }
  ...
  return () => {
    ...
    const handlePagebreakChange = async (event) => {
      ...
      chapterPagebreakOrLandscape.push(isLandscape ? 'Y' : 'N')
        let response = await updateImplchapterPagebreakAndLandscape(chapterPagebreakOrLandscape);
      };
    ...
    const getAllChapters = async (projectSelectedDtoData,editorFlag,versionElementId) => {
        try {
            ...
            let AllChapters = await getImplMainChapters(projectSelectedDtoData,versionElementId.docVersionReleaseId);
    ...
    const getSubChapters = async (chapterId, level) => {
        ...
        try {
            ...
            const response = await getImplSubChaptersById(chapterId);
            if (level === 1) {
                const initialChapterSnNos = new Map();
    ...
    const updateChapterName = async(chapterId,versionElementId) => {
        ...
        if (confirm) {
                ...
                let res = await updateImplChapterNameById(chapterName);
                if (res && res > 0) {
    ...
    const afterSubmit = async(level0fSnNo) => {
        ...
        if(level0fSnNo === 1){
            ...
            setChapterLv1SnNos(new Map());
            const initialChapterSnNos = new Map();
            const response = await getImplSubChaptersById(chapterSnId);
            (response || []).forEach(item =>{
                initialChapterSnNos.set(item[0],item[7])
            })
            setChapterLv1SnNos(new Map(initialChapterSnNos));
    ...
    const deleteChapterById = async(reloadChapterId, chapterId, level) => {
        ...
        if (confirm) {
            let res = await deleteImplChapterByChapterId(chapterId);
            if (res && res > 0) {
                if (level > 0) {
                    if (level === 1) {
                        setChapterIdFirstLvl(null);
    ...
    const updateEditorContent = async() => {
        ...
        if (confirm) {
            ...
            let res = await updateImplChapterContent(chaperContent);
    ...
    const submitAddSubChapterForm = async(chapterId, level, serialNoAdd) => {
        ...
        try {
            ...
            let res = await addImplSubChapterNameByChapterId(ChapterNameAndId);
    ...
    const handleSnSubmit = async(level) => {
        ...
        if (confirm) {
                const response = await updateImplChapterBySnNo((level === 1?chapterSnNos:level === 2?chapterLv1SnNos:chapterLv2SnNos));
    ...
    switch (status) {
        case 'abbreviation':
            ...
            case 'list':
                return <ImplementationDocRecordsComponent  projectId={versionElements.projectId} ></ImplementationDocRecordsComponent>;
          case 'addTableContent':
             return <ImplDocTableComponent versionElements={versionElements}></ImplDocTableComponent>
        default:
        return (
            ...
            <Box id="main-container" className='main-container'>
               <Box id="main-breadcrumb">
                    <Breadcrumbs separator=">" aria-label="breadcrumb" className="row headingLink">
                        ...
                        href="/implementation-document"
                        >
                            <MdAccountTree></MdAccountTree> Implementation Record
                        </Link>
                        <Typography color="textPrimary"> Implementation Doc Content </Typography>
                    </Breadcrumbs>
                </Box>
                ...
                <Box id="main-wrapper">
                   ...
                   <ImplDocsAddDocContentAddSectionDialog 
                        open={openDialog}
...
export default withRouter(ImplAddContentEditor);