...
import  implDocSections,{ addImplNewChapterSection, getImplUnAddedChapterlist, ImplAddListToAddList } from 'services/implementation.service';
...
const UserManualDocsAddDocContentAddSectionDialog = ({ open, onClose, versionElements,snNo }) => {
    ...
    const getUnAddedChapterlist = async (projectSelDtoArg) => {
        try {
            let unAddedChapterList = await getImplUnAddedChapterlist(projectSelDtoArg);
    ...
    const handleCheckboxChange = (id, event) => {
      ...
      const submitConfirmation = async() => {
        ...
        let res = await ImplAddListToAddList(new AddSectionIdsDto(sectionIds,snNo));
       //  console.log('res reult :'+res);
        if (res) {
            getImplUnAddedChapterlist(projectSelDto);
            Swal.fire({
                icon: "success",
                title: 'Success',
      ...
      const addConfirmation = async() => {
        ...
        if (confirm) {
            ...
            let docSections = new implDocSections(0, newSectionName, projectSelDto.projectId,null,null,null,null, 1,docVersionReleaseId.releaseId);

            let res = await addImplNewChapterSection(docSections);
